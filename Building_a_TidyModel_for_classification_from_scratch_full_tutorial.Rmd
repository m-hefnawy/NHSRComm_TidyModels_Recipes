...
strand_pat <- read_csv("Data/Stranded_Data.csv") %>% 
  ...
  mutate(frail_descrip = factor(frail_descrip)) %>% 
  drop_na()
...
stranded_rec %>% prep() %>% juice() %>% glimpse()
...
lr_mod <- 
  parsnip::logistic_reg(penalty = 0.5, mixture = 0.5) %>% 
  set_engine("glm")
...
plot <- strand_fitted %>% 
  ...
  labs(y="P value", x="Terms",                                                              title="P value significance chart",                                                  subtitle="A chart to represent the significant variables in the model",
...
# Improving the model with another different model like XGboost

The following example will utilize XGBoosting which is an ensebmle powerful algorithm

## Define and instantiate the model

The first step, as with the logistic regression example, if to define and instantiate the model:

```{r define_xgboost model}
...
xg_fit <- boost_tree_xgboost_spec %>% 
    fit(stranded_class ~ ., data = train_data)
...
```{r fit_xg_boost_model_rsamples_preds}
xg_resample_mean_preds <- tune::collect_metrics(xg_fit_rs)
print(xg_resample_mean_preds)
```
...
# Improve random forest model with hyperparameter tuning with the Dials package

We are going to now create a random forest model and we are going to tune the hyperparameters using the dials package. The dials package contains a list of hyperparameter tuning methods and is useful for creating quick hyperparameter grids and aiming to optimise them.

## Building the random forest

Like all the other steps, the first thing to do is build the random forest Note - the reason set_model("classification") is because the thing we are predicting is a factor. If this was a continuous variable, then you would need to switch this to regression. However, the model development for regression is identical to classification.
...
The next step is to fill these blank values for mtry, trees, and min_n using grid_max_entropy

We will now create the object:

```{r build_tuning_rf}
grid_rf_tune <- grid_max_entropy(
    mtry(range = c(1,20)),
    trees(range = c(50, 500)),
    min_n(range = c(1,20)),
    size = 30,
    iter = 1000)
print(head(grid_rf_tune,20))
```
...
rf_pred_tuned <- 
  ...
  tune::tune_grid(
    resamples = ten_fold, #This is the 10 fold cross validation variable we created earlier
    grid = grid_rf_tune #This is the tuning grid
  )
...
I tried to mimic the visualization with the random forest model but it looks weird!!

```{r tune_visual_rf}
tune_rf_plot <- rf_pred_tuned %>%
  collect_metrics() %>% #Collect metrics from tuning
...
print(tune_rf_plot)
ggsave(filename="Figures/hyperparameter_rf.png", tune_rf_plot)
...
The next step is to us the best rf to make our predictions.

## Using best tree to make predictions

```{r tune_best_select_rf}
final_rf_wf <- 
  rf_wf %>% 
  finalize_workflow(best_rf) #Finalise workflow passes in our best tree

print(final_rf_wf)

```

Make a prediction against this finalised rf:

```{r tune_best_rf_selected}
final_rf_pred <- 
  final_rf_wf %>% 
  fit(data = train_data)
...
```{r wf_rf_fit}
plot_rf <- final_rf_pred %>% 
  pull_workflow_fit() %>% 
  vip(aesthetics = list(color = "black", fill = "#26ACB5")) + theme_minimal()

print(plot_rf)
ggsave("Figures/VarImp_rf.png", plot_rf)
...
final_rf_fit_fitted_metrics <- final_rf_fit %>% 
  collect_metrics() 

print(final_rf_fit_fitted_metrics)

#Create the final predictions
final_rf_fit_predictions <- final_rf_fit %>% 
  collect_predictions()
print(final_rf_fit_predictions)
...
```{r final_rf_fit}
roc_plot_rf <- final_rf_fit_predictions %>% 
  roc_curve(stranded_class, `.pred_Not Stranded`) %>% 
  autoplot()

print(roc_plot_rf)
ggsave(filename = "Figures/tuned_rf.png", plot=roc_plot_rf)

```

# Improve XGBoost model with hyperparameter tuning with the Dials package

We are going to now create a XGBoost model and we are going to tune the hyperparameters using the dials package. The dials package contains a list of hyperparameter tuning methods and is useful for creating quick hyperparameter grids and aiming to optimise them.

## Building the Tuned XGBoost Model

Like all the other steps, the first thing to do is build the XGBoost model Note - the reason set_model("classification") is because the thing we are predicting is a factor. If this was a continuous variable, then you would need to switch this to regression. However, the model development for regression is identical to classification.

```{r build_tune_xgboost}
tune_xgboost <- 
...
print(head(grid_xgboost_tune,20))
```

## Setting up parallel processing

The tuning process, and modeling process, normally needs the ML engineer to access the full potential of your machine. The next steps show how to register the cores on your machine and max them out for training the model and doing grid searching:

```{r parallelproc_xgboost}
all_cores <- parallel::detectCores(logical = FALSE)-1
print(all_cores)
#Registers all cores and subtracts one, so you have some time to work
cl <- makePSOCKcluster(all_cores)
...
xgboost_pred_tuned <- 
  ...
  tune::tune_grid(
    resamples = ten_fold, #This is the 10 fold cross validation variable we created earlier
    grid = grid_xgboost_tune #This is the tuning grid
  )
...
```{r tune_visual_xgboost}
tune_xgboost_plot <- xgboost_pred_tuned %>%
  collect_metrics() %>% #Collect metrics from tuning
  mutate(trees = factor(trees)) %>%
  ggplot(aes(mtry, mean, color = trees)) +
  geom_line(size = 1, alpha = 0.7) +
...
print(tune_xgboost_plot)
ggsave(filename="Figures/hyperparameter_xg.png", tune_xgboost_plot)
...
# Select the best tree
best_xgboost <- xgboost_pred_tuned %>% 
  tune::select_best("roc_auc")
...
```{r tune_best_select_xgboost}
final_xgboost_wf <- 
  xgboost_wf %>% 
  finalize_workflow(best_xgboost) #Finalise workflow passes in our best tree

print(final_rf_wf)

```

Make a prediction against this finalized xgboost:

```{r tune_best_xgboost_selected}
final_xgboost_pred <- 
  final_xgboost_wf %>% 
  fit(data = train_data)

print(final_xgboost_pred)
...
```{r wf_xgboost_fit}
plot_xgboost <- final_xgboost_pred %>% 
  pull_workflow_fit() %>% 
  vip(aesthetics = list(color = "black", fill = "#26ACB5")) + theme_minimal()

print(plot_xgboost)
ggsave("Figures/VarImp_xgboost.png", plot_xgboost)
...
```{r final_xgboost_fitted}
# Create the final prediction
final_xgboost_fit <- 
  final_xgboost_wf %>% 
  last_fit(split)

final_xgboost_fit_fitted_metrics <- final_xgboost_fit %>% 
  collect_metrics() 

print(final_xgboost_fit_fitted_metrics)

#Create the final predictions
final_xgboost_fit_predictions <- final_xgboost_fit %>% 
  collect_predictions()
print(final_xgboost_fit_predictions)
...
```{r final_xgboost_fit}
roc_plot_xgboost <- final_xgboost_fit_predictions %>% 
  roc_curve(stranded_class, `.pred_Not Stranded`) %>% 
  autoplot()

print(roc_plot_xgboost)
ggsave(filename = "Figures/tuned_xgboost.png", plot=roc_plot_xgboost)
